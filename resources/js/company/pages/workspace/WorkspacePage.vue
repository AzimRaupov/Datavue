...
/*
 * .flex-fill даёт "flex: 1 1 auto !important" — базовый размер инпута
 * auto, то есть его "естественная" (нерастянутая) ширина. Перенос строки
 * решается ДО распределения свободного места по flex-grow, по этим самым
 * базовым размерам: браузер видит, что инпут + бейдж + селект + кнопка не
 * помещаются в строку по их базовым ширинам, и переносит остаток (селект,
 * кнопку) под заголовок — хотя после grow инпуту физически хватило бы места.
 * flex-basis: 0 убирает инпуту "естественную" ширину из этого расчёта:
 * с не растянутым 0 всё остальное всегда помещается на одной строке,
 * а всё свободное место инпут получает уже потом, через flex-grow.
 * !important обязателен — иначе проигрывает important-у самого .flex-fill.
 */
.builder-title {
    cursor: text;
    min-width: 0;
